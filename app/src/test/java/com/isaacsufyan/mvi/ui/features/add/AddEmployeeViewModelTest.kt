...
import com.isaacsufyan.mvi.utils.BaseJunit4Test
...
class AddEmployeeViewModelTest : BaseJunit4Test() {
    ...
    fun `on SaveEmployeeEvent when form is valid should show snackbar and clear form`() = runBlockingTest {
        ...
        with(viewModel) {
            with(viewModel.snackbarState.value) {
    ...
    fun `on SaveEmployeeEvent when form is not valid should show error snackbar`() =
        runBlockingTest {
            ...
            assertThat(viewModel.snackbarState.value.isShown).isTrue()
            assertThat(viewModel.snackbarState.value.message).isEqualTo("Form is not valid")
        }
